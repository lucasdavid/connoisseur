...
ex = Experiment('inception-auxiliary-classifier-pca-svm')
...
def run(_run, dataset_seed, svm_seed,
    ...
    with tf.device(device):
        ...
        avg_pooling2D = layers.AveragePooling2D((8, 8), strides=(8, 8), name='avg_pool')
        flat = layers.Flatten(name='flatten')

        x = layers.merge([
            flat(avg_pooling2D(base_model.output)),
            flat(avg_pooling2D(base_model.get_layer('mixed4').output)),
            flat(avg_pooling2D(base_model.get_layer('mixed7').output)),
        ], mode='concat')
