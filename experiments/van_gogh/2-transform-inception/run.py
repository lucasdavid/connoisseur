...
def run(dataset_seed, image_shape, batch_size, device, data_dir,
    ...
    with tf.device(device):
        network = InceptionV3(input_shape=image_shape, include_top=False)

    g = ImageDataGenerator(rescale=2. / 255., featurewise_center=True)
    g.mean = 1

    for phase in ('train', 'valid', 'test'):
        phase_data_dir = os.path.join(data_dir, 'vgdb_2016', 'extracted_patches', phase)
        output_file_name = os.path.join(data_dir, 'vgdb_2016', '%s.pickle' % phase)
        ...
        while total_samples_seen < data.N:
            _X, _y = next(data)
            _X = network.predict_on_batch(_X)
        ...
        with open(output_file_name, 'wb') as f:
            pickle.dump({'data': X, 'target': y,
                         'names': np.array(data.filenames, copy=False)},
