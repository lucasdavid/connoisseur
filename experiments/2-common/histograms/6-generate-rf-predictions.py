...
ex = Experiment('generate-rf-predictions')
...
def run(_run, data_dir, phases, classes, layer, ckpt,
    ...
    data = map(lambda x: (x['data'], x['target'], x['names']),
               [joblib.load(os.path.join(data_dir, '%s.0.pickle' % phase))
                for phase in phases])

    data = dict(zip(phases, data))
    print('done.')
